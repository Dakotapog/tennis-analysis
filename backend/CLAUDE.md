...
> Last updated: 2026-07-27 (Nodo-145: Pipeline Bugs tipo_cancha+timing — D145-01: h2h_extractor tipo_cancha usa superficie como fallback + copia hora. D145-02: edge_calculator timing guard skip partidos >15min pasados. Root cause cascada 0 combos: tipo_cancha='N/A' → unknown bucket 24% hit → Kelly aplastado → pipeline_picks vacío → 0 CORE. 7 tests REGLA-T53. 2314 tests.) Nodo-144: Trazabilidad de Estrategia en Shadow Book — D144-01→D144-07. Campo 'strategy' top-level en _build_record(). tag_strategy(). log_pick() singular. segmento strategy en report(). backfill_strategy.py retroactivo (22 tags alta confianza, 1287 HISTORICO_SIN_TAG). 7 tests REGLA-T53. 2307 tests.)
