...
def _make_gsr(partidos, tmp_path):
...
# ─── Test 4: Convergencia ≥2 ALTA EN_VIVO → Popen ───────────────────────────

def test_convergencia_2_alta_dispara(tmp_path):
    """2 señales ALTA en STARTED → convergencia_activa=True → Popen llamado 1x."""
    _make_gsr([
...
def test_antiflood_no_refire(tmp_path):
    """frozenset {A,B} ya en fired → Popen NO llamado segunda vez."""
    fecha = datetime.now().strftime("%Y-%m-%d")
