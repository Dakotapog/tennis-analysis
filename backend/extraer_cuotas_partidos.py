...
class ZitaScraper:
    ...
    async def extract_single_match(self, element):
        ...
        try:
            ...
            # Extraer cuotas con selectores específicos del HTML de FlashScore
            try:
                # Selectores específicos primero (más precisos)
                odd1_el = await element.query_selector('.event__odd--odd1')
                odd2_el = await element.query_selector('.event__odd--odd2')

                # Fallback: selectores genéricos si los específicos no funcionan
                ...
                odds_elements = []
                if odd1_el and odd2_el:
                    odds_elements = [odd1_el, odd2_el]
                ...
                # Validar que las cuotas sean realistas (> 1.0)
                if match_data['cuota1'] and match_data['cuota1'] <= 1.0:
                    match_data['cuota1'] = None
                if match_data['cuota2'] and match_data['cuota2'] <= 1.0:
                    match_data['cuota2'] = None
